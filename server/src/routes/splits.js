...
async function computeBalances(groupId) {
  ...
  // Settlements: from pays to → from balance decreases debt (from gives money)
  // from_member pays to_member: from loses money (balance ↓), to gains (balance ↑)
  // Actually in splitwise: if A pays B to settle, A's debt decreases (A's net goes up toward 0 if negative)
  // Standard: balance = paid - share
  // When A pays settlement to B: A paid more, so A.balance += amount; B.balance -= amount
