 ...
 * Keeps the purple card layout users liked — no logo images and no links
 * (remote images / duckdns URLs hurt inbox delivery).
 */
export function buildOtpEmail({ otp, purpose = 'signup', name }) {
  const isReset = purpose === 'reset';
  const title = isReset ? 'Reset your password' : 'Verify your email';
  const greetingPlain = name ? `Hi ${String(name).trim()},` : 'Hi there,';
  ...
  // Text mark only — no <img>, no cid:, no https links.
  const brandMark = `
    <div style="width:64px;height:64px;border-radius:16px;margin:0 auto 12px;background:rgba(255,255,255,0.18);line-height:64px;text-align:center;color:#ffffff;font-size:28px;font-weight:800;font-family:Arial,Helvetica,sans-serif;">S</div>
  `;
